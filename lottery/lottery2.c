...
int main() {
	...
	int ***input = (int ***)malloc(sizeof(int **) * caseNum);
	for(int i = 0; i < caseNum; i++){
		scanf("%d", &groupNum);
		input[i] = (int **)malloc(sizeof(int *) * groupNum);
		for(int j = 0; j < groupNum; j++){
			input[i][j] = (int *)malloc(sizeof(int) * 3);
            scanf("%d %d %d", &input[i][j][0], &input[i][j][1], &input[i][j][2]);
		}
	}
